// Auth mutations
export const SET_AUTH = 'setAuth'
export const REMOVE_AUTH = 'removeAuth'
export const SET_ERROR = 'setError'

// User mutations
export const SET_USERS = 'setUsers'
export const SET_USER = 'setUser'
export const ADD_USER_IN_LIST = 'addUserInList'
export const UPDATE_USER_IN_LIST = 'updateUserInList'
export const DELETE_USER_IN_LIST = 'deleteUserInList'
export const SET_USER_ERROR = 'setUserError'
export const SET_USER_INIT = 'setUserInit'

// Record mutations
export const SET_RECORDS = 'setRecords'
export const SET_RECORD = 'setRecord'
export const ADD_RECORD_IN_LIST = 'addRecordInList'
export const UPDATE_RECORD_IN_LIST = 'updateRecordInList'
export const DELETE_RECORD_IN_LIST = 'deleteRecordInList'
export const SET_RECORD_ERROR = 'setRecordError'
export const SET_RECORD_INIT = 'setRecordInit'
export const SET_REPORT = 'setReport'
export const SET_FROM = 'setFrom'
export const SET_TO = 'setTo'
